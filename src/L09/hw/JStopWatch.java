...
public class JStopWatch extends JFrame implements Runnable {
	...
	private void ClockPanel() {
		...
		p_display.setBackground(Color.orange);
	...
	private void ButtonPanel() {
		...
		// 이벤트 리스너 설정
		bStart.addActionListener(h1);
		bPause.addActionListener(h1);
	...
	// 리스너 처리
	private ActionListener h1 = new ActionListener() {
	...
	public void run() {
		while (thread != null) {
			...
			try {
				...
				if ((Integer.parseInt(l_msec.getText()) + 1) % 100 == 0) { // 99msec일때 sec의 변화
					if (Integer.parseInt(l_sec.getText()) < 9) { // 현재 sec가 두 자리수가 아니면
						...
						if ((Integer.parseInt(l_sec.getText()) + 1) % 60 == 0) { // 59sec일때 min의 변화
